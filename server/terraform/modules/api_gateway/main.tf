...
# グループ内のチャット履歴取得リソース
resource "aws_api_gateway_resource" "groups_chat" {
  rest_api_id = aws_api_gateway_rest_api.api.id
  parent_id   = aws_api_gateway_resource.groups.id
  path_part   = "chat"
}

resource "aws_api_gateway_resource" "chat_groupId" {
  rest_api_id = aws_api_gateway_rest_api.api.id
  parent_id   = aws_api_gateway_resource.groups_chat.id
  path_part   = "{groupId}"
}
...
# 回答にいいねをするリソース
resource "aws_api_gateway_resource" "answer_like" {
  rest_api_id = aws_api_gateway_rest_api.api.id
  parent_id   = aws_api_gateway_resource.answers.id
  path_part   = "like"
}

resource "aws_api_gateway_resource" "like_answerId" {
  rest_api_id = aws_api_gateway_rest_api.api.id
  parent_id   = aws_api_gateway_resource.answer_like.id
  path_part   = "{answerId}"
}
...
# グループ内のチャット履歴取得メソッド
resource "aws_api_gateway_method" "chat_groupId" {
  rest_api_id   = aws_api_gateway_rest_api.api.id
...
# 回答へのlikeメソッド
resource "aws_api_gateway_method" "like_answerId_put" {
  rest_api_id   = aws_api_gateway_rest_api.api.id
  resource_id   = aws_api_gateway_resource.like_answerId.id
  http_method   = "PUT"
  authorization = "COGNITO_USER_POOLS"
  authorizer_id = aws_api_gateway_authorizer.cognito.id
}
...
# 回答へのlike解除メソッド 
resource "aws_api_gateway_method" "like_answerId_delete" {
  rest_api_id   = aws_api_gateway_rest_api.api.id
  resource_id   = aws_api_gateway_resource.like_answerId.id
  http_method   = "DELETE"
  authorization = "COGNITO_USER_POOLS"
...
# グループ内のチャット履歴取得の統合
resource "aws_api_gateway_integration" "chat_groupId" {
  rest_api_id             = aws_api_gateway_rest_api.api.id
  resource_id             = aws_api_gateway_resource.chat_groupId.id
  http_method             = aws_api_gateway_method.chat_groupId.http_method
  integration_http_method = "POST"
  type                    = "AWS_PROXY"
  uri                     = "arn:aws:apigateway:${var.aws_region}:lambda:path/2015-03-31/functions/${var.lambda_get_history_arn}/invocations"
}
...
# 回答にいいねをする統合
resource "aws_api_gateway_integration" "like_answerId_put" {
  rest_api_id             = aws_api_gateway_rest_api.api.id
  resource_id             = aws_api_gateway_resource.like_answerId.id
  http_method             = aws_api_gateway_method.like_answerId_put.http_method
  integration_http_method = "POST"
  type                    = "AWS_PROXY"
...
# 回答にいいねを解除する統合
resource "aws_api_gateway_integration" "like_answerId_delete" {
  rest_api_id             = aws_api_gateway_rest_api.api.id
  resource_id             = aws_api_gateway_resource.like_answerId.id
  http_method             = aws_api_gateway_method.like_answerId_delete.http_method
  integration_http_method = "POST"
  type                    = "AWS_PROXY"
  uri                     = "arn:aws:apigateway:${var.aws_region}:lambda:path/2015-03-31/functions/${var.lambda_like_arn}/invocations"
}
...
# 回答にいいねをする用
resource "aws_api_gateway_method" "like_answerId_options" {
  rest_api_id   = aws_api_gateway_rest_api.api.id
  resource_id   = aws_api_gateway_resource.like_answerId.id
  http_method   = "OPTIONS"
  authorization = "NONE"
...
resource "aws_api_gateway_integration" "like_answerId_options" {
  rest_api_id   = aws_api_gateway_rest_api.api.id
  resource_id   = aws_api_gateway_resource.like_answerId.id
  http_method   = aws_api_gateway_method.like_answerId_options.http_method
  type          = "MOCK"
  request_templates = {
    "application/json" = jsonencode({
      statusCode = 200
...
resource "aws_api_gateway_method_response" "like_answerId_options" {
  rest_api_id   = aws_api_gateway_rest_api.api.id
  resource_id   = aws_api_gateway_resource.like_answerId.id
  http_method   = aws_api_gateway_method.like_answerId_options.http_method
  status_code   = "200"
...
resource "aws_api_gateway_integration_response" "like_answerId_options_integration_response" {
  rest_api_id   = aws_api_gateway_rest_api.api.id
  resource_id   = aws_api_gateway_resource.like_answerId.id
  http_method   = aws_api_gateway_method.like_answerId_options.http_method
  status_code   = aws_api_gateway_method_response.like_answerId_options.status_code
  ...
  depends_on = [aws_api_gateway_integration.like_answerId_options]
}
...
locals {
  ...
  group_resources = [
    ...
    aws_api_gateway_resource.chat_groupId.id,
  ...
  answers_resources = [
    ...
    aws_api_gateway_resource.like_answerId.id,
  ]
  ...
  api_methods = [
    ...
    aws_api_gateway_method.chat_groupId.id,
    ...
    aws_api_gateway_method.like_answerId_put.id,
    aws_api_gateway_method.like_answerId_delete.id
  ]
  ...
  api_integrations = [
    ...
    aws_api_gateway_integration.chat_groupId.id,
    ...
    aws_api_gateway_integration.like_answerId_put.id,
    aws_api_gateway_integration.like_answerId_delete.id
  ]
...
resource "aws_api_gateway_deployment" "api_deployment" {
  ...
  depends_on = [
    ...
    aws_api_gateway_integration.chat_groupId,
    ...
    aws_api_gateway_integration.like_answerId_put,
    aws_api_gateway_integration.like_answerId_delete,
    # CORS統合レスポンス
...
resource "aws_lambda_permission" "get_history_permission" {
  statement_id  = "AllowAPIGatewayInvoke_GetHistory"
  action        = "lambda:InvokeFunction"
